...
@bp.route('/products/print')
@login_required
def print_products():
    """Print products list"""
    products = Product.query.filter_by(is_active=True).order_by(Product.sku).all()
    return render_template('inventory/products_print.html', products=products)

@bp.route('/products/email', methods=['POST'])
@login_required
def email_products():
    """Email products report"""
    try:
        ...
        headers = ['SKU', 'Name', 'Category', 'Unit', 'MRP', 'Base Price', 'GST %']
        for col, header in enumerate(headers, 1):
        ...
        for row_idx, prod in enumerate(products, 2):
            ...
            ws.cell(row=row_idx, column=3, value=prod.category.name if prod.category else '')
            ws.cell(row=row_idx, column=4, value=prod.unit)
            ws.cell(row=row_idx, column=5, value=prod.mrp)
            ws.cell(row=row_idx, column=6, value=prod.base_price)
            ws.cell(row=row_idx, column=7, value=prod.gst_rate)
        ...
        msg = Message(
            subject='Products Catalog - Mohi Industries',
            recipients=[recipient],
            body=f'Products catalog as of {datetime.now().strftime("%d-%m-%Y %H:%M")}\n\nTotal Products: {len(products)}'
        )
        ...
        return jsonify({'success': True, 'message': f'✅ Products report sent to {recipient}'})
        
    except Exception as e:
        return jsonify({'success': False, 'message': f'❌ Error: {str(e)}'}), 500

@bp.route('/products/whatsapp', methods=['POST'])
@login_required
...
'''
        ...
        for prod in products[:20]:
            message += f"{prod.sku} - {prod.name}\n"
            message += f"   MRP: ₹{prod.mrp} | GST: {prod.gst_rate}%\n\n"
        ...
        return jsonify({'success': True, 'message': f'✅ Products catalog sent to {phone} via WhatsApp'})
